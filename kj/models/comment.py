...
class Comment(Base, KJBase):
    ...
    product = relationship('Product', backref=backref('comments', order_by="Comment.when_created.desc()", lazy='dynamic'))
    ...
    def details(self):
        return "dodany %s przez %s" % (self.when_created.strftime('%d/%m/%Y %H:%M'), self.user.user_name())
