...
@view_config(route_name='inbox', renderer='kj:templates/front/my_messages.html', permission='user')    
def inbox(request):
...
@view_config(route_name='outbox', renderer='kj:templates/front/my_messages.html', permission='user')    
def outbox(request):
    threads_i_sent = request.user.get_active_sender_threads()
    threads_i_get = request.user.get_active_recipient_threads()
    get_unread = 0
...
@view_config(route_name='archive', renderer='kj:templates/front/my_messages.html', permission='user')    
def archive(request):
...
@view_config(route_name='show_thread_messages', renderer='kj:templates/front/message.html', permission='user')
def show_thread_messages(request):
    ...
    return {
        ...
        'title' : u'<a class="top-shelf" href="%s">Wiadomości</a> &raquo; %s' % (request.route_path('my_threads'), smart_truncate(thread.product.name, 65))
    }
...
def send_message(request):
    ...
    existing_thread = Thread.find_by_sender_and_recipient_and_product(sender_id, recipient_id, product_id)
    if existing_thread:
    ...
    if len(thread.messages) > 1:
        return HTTPFound(location=request.route_path('show_thread_messages', id=thread.id, _anchor='last_or_new'))
    else:
        return HTTPFound(location=request.referrer)
